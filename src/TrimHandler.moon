class TrimHandler
	...
	performTrim: =>
		with platform = ({
				[true]:  {pre: os.getenv('TEMP'), ext:'.bat', exec:'""%s""',  postexec:'\nif errorlevel 1 (echo Error & pause)'}
				[false]: {pre: "/tmp",            ext:'.sh',  exec:'sh "%s"', postexec:' 2>&1'}
			})[@windows]
			...
			sh\close!
			os.execute .exec\format encsh

		-- output = io.popen .exec\format encsh
